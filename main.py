...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QAbstractItemView, QListView, QTabWidget)
...
class PlaywrightWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit("ERROR", f"Worker Thread Exception: {str(e)}")
            if hasattr(self.bot, "update_status"):
                try:
                    self.bot.update_status(self.row_id, "FAILED", f"Crash: {str(e)[:40]}")
                except:
                    pass
            self.bot.last_error_reason = f"CRASH: {str(e)[:80]}"
        finally:
            self.error_reason = getattr(self.bot, "last_error_reason", None)
            self.finished_signal.emit(self.row_id, bool(success))
...
class NexusAutomatorWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("AKIRA PRO — AI Video Generator")
        ...
        self.row_accounts = {}     # Map of row_id to selected account name
        self.img_queue_data = []
        ...
        self.session_bandwidth_bytes = 0
    ...
    def init_ui(self):
        ...
        details = QVBoxLayout()
        self.lbl_plan_name = QLabel("AKIRA")
        self.lbl_plan_name.setObjectName("ProfileName")
        self.lbl_plan_name.setStyleSheet("font-size: 8.5pt; border: none; background: transparent; font-weight: bold;")
        ...
        details.addWidget(self.lbl_plan_name)
        ...
        self.btn_help.setObjectName("HeaderActionButton")
        self.btn_help.setText("📖")
        self.btn_help.setToolTip("Quick Start Guide / User Manual")
        ...
        self.btn_help2 = QPushButton()
        self.btn_help2.setObjectName("HeaderActionButton")
        self.btn_help2.setText("📖")
        self.btn_help2.setToolTip("Quick Start Guide / User Manual")
        ...
        self.cb_use_direct_api = QCheckBox("Use Fast API Mode (Bypasses UI)")
        self.cb_use_direct_api.setChecked(False)
        ...
        self.table.setHorizontalHeaderLabels(["#", "Prompt (Preview)", "Account", "Status", "Action", ""])
        ...
        self.table.setColumnWidth(3, 105) # Fixed 105px width for Status column to prevent badge text wrapping/clipping
        ...
        self.btn_ctrl_stop = QPushButton("⏹ Stop Queue")
        self.btn_ctrl_stop.setObjectName("StopBtn")
        self.btn_ctrl_stop.setFixedWidth(135)
        ...
        queue_btn_layout.addWidget(self.btn_ctrl_stop)
        ...
        self.btn_add_queue.setObjectName("PromptActionButton")
        ...
        prompts_hdr_lay.addWidget(self.btn_add_queue)
        ...
        # Left Console: All Logs
        self.console = QTextEdit()
        self.console.setObjectName("ConsoleTerminal")
        self.console.setReadOnly(True)
        
        # Right Console: Errors & Warnings
        self.error_console = QTextEdit()
        self.error_console.setObjectName("ConsoleTerminal")
        self.error_console.setStyleSheet("QTextEdit#ConsoleTerminal { border: 1px solid #5D2B34; }")
        self.error_console.setReadOnly(True)
        
        self.console_tabs.addTab(self.console, " All Logs")
        self.console_tabs.addTab(self.error_console, " Errors & Warnings")
        
        logs_layout.addWidget(self.console_tabs)
    ...
    def connect_actions(self):
        ...
        if hasattr(self, "btn_help"):
            self.btn_help.clicked.connect(self.show_user_manual)
        if hasattr(self, "btn_help2"):
            self.btn_help2.clicked.connect(self.show_user_manual)
        if hasattr(self, "btn_help3"):
        ...
        self.btn_add_queue.clicked.connect(self.add_prompts_to_queue)
        ...
        self.btn_ctrl_stop.clicked.connect(self.control_stop_clicked)
        ...
        # Connect speed inputs to recalculate estimated times on text change
        self.inp_speed.textChanged.connect(self.recalculate_stats)
        self.inp_img_speed.textChanged.connect(self.recalculate_img_stats)
    ...
    def control_stop_clicked(self):
        if not self.is_generating and not self.workers:
            self.log("WARNING", "Generation is not active. Nothing to stop.")
            # Verify if there are any orphaned or leftover workers just in case
            if not self.workers:
                return
            
        reply = QMessageBox.question(self, "Stop Generation Queue",
            "Are you sure you want to forcibly stop all running browser processes and abort generation?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            
        if reply == QMessageBox.StandardButton.Yes:
            self.log("SYSTEM", "Forcibly stopping all generation tasks and closing Chrome browsers...")
            self.stop_all_workers()

    def stop_all_workers(self):
        active_workers = list(self.workers.values())
        for worker in active_workers:
            # Disconnect signals to prevent shutdown callbacks from interfering
            try:
            ...
            # Set stop flag on the bot
            worker.bot.is_stopped = True
            
            # Close browser context instantly
            if hasattr(worker.bot, "_current_context") and worker.bot._current_context:
                try:
                    worker.bot._current_context.close()
                except Exception:
                    pass
            
            # Forcibly terminate QThread if it doesn't shut down in 1 second
            worker.quit()
            worker.wait(1000)
            if worker.isRunning():
                worker.terminate()
        ...
        # Reset statuses of generating/submitting/waiting items back to QUEUED
        for item in self.queue_data:
            if item["status"] in ["GENERATING", "SUBMITTING", "WAITING"]:
                item["status"] = "QUEUED"
                self.action_text_cache[item["id"]] = "Stopped by user"
                
        self._reset_generation_ui()
        self.reorder_table_rows()
        self.log("SUCCESS", "All processes forcibly stopped.")
    ...
    def log(self, type_str, message):
        # Write to raw log file for diagnostics
        try:
            with open("raw_app.log", "a", encoding="utf-8") as f:
                f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [{type_str}] {message}\n")
        except:
            pass

        # Extract row account name from logs
        if "Selected Account" in message:
            try:
                parts = message.split("Selected Account")
                acc_part = parts[1].strip().split()[0]
                acc_clean = acc_part.replace(".json", "")
                if ":" in parts[0] and "Row" in parts[0]:
                    rid_str = parts[0].replace("Row", "").replace(":", "").strip()
                    row_id = int(rid_str)
                    self.row_accounts[row_id] = acc_clean
                    # Force redraw of table row so new account name is instantly visible
                    QTimer.singleShot(0, self.reorder_table_rows)
            except Exception as e:
                print(f"Error parsing Selected Account: {e}")
        ...
        if self.client_mode:
            ...
            if type_str == "INFO":
                ...
                elif ("Still waiting for video" in message or "Still waiting for generation" in message) and "[DIAG" not in message:
                    is_important = True
            ...
            # 4. Filter SUCCESS logs
                ...
                if "CONFIRMED:" in message:
                    ...
                    is_important = True
                elif "Total network bandwidth consumed" in message:
                    is_important = True
            ...
            # 5. Filter WARNING logs
                ...
                is_important = False
                if "Content policy violation" in message or "warning" in message.lower() or "aborted" in message.lower() or "Removed from queue" in message or "Total network bandwidth consumed" in message:
                    is_important = True
        ...
        if type_str in ("ERROR", "WARNING"):
            self.error_console.append(log_line)
    ...
    def show_user_manual(self):
        dlg = AkiraHelpDialog(self)
        dlg.exec()
        
    def update_license_display(self, plan, time_left_sec):
        plan_upper = plan.upper()
        
        # 1. Formatting remaining time (hours hidden when days >= 1)
        try:
        ...
        if plan.lower() == "lifetime" or time_left_sec > 10 * 365 * 24 * 3600:
            ...
            if time_left_sec >= sec_in_year:
                ...
                parts = []
                # Normalize months to years (e.g. 12 months -> 1 year)
                if months >= 12:
                    years = months // 12
                    months = months % 12
                    if years > 0: parts.append(f"{years}y")
        ...
        # Update text values
        if plan_upper == "PLATINUM":
            self.lbl_plan_name.setText("💎 PLATINUM")
        else:
            self.lbl_plan_name.setText(f"{plan_upper} PLAN")
            
        self.lbl_sub.setText(time_str)
        
        # 2. Applying Colors & Styles
        if plan_upper == "TRIAL":
            self.lbl_plan_name.setStyleSheet("font-size: 8.5pt; font-weight: bold; color: #9CA3AF; border: none; background: transparent;")
            self.lbl_sub.setStyleSheet("font-size: 7.5pt; color: #6B7280; border: none; background: transparent;")
        elif plan_upper == "BASIC":
            self.lbl_plan_name.setStyleSheet("font-size: 8.5pt; font-weight: bold; color: #3B82F6; border: none; background: transparent;")
            self.lbl_sub.setStyleSheet("font-size: 7.5pt; color: #60A5FA; border: none; background: transparent;")
        elif plan_upper == "PREMIUM":
            self.lbl_plan_name.setStyleSheet("font-size: 8.5pt; font-weight: bold; color: #C5A059; border: none; background: transparent;")
            self.lbl_sub.setStyleSheet("font-size: 7.5pt; color: #E5C17A; border: none; background: transparent;")
        elif plan_upper == "PLATINUM":
            # Extra premium look: diamond purple/violet style with bold weight
    ...
    def filter_logs(self, query):
        ...
        err_lines = self.error_console.toPlainText().split('\n')
        self.error_console.clear()
        for l in err_lines:
            if query in l.lower():
                self.error_console.append(l)
    ...
    def delete_row(self, row_id):
        ...
        # Abort worker thread if active
        if status in ["SUBMITTING", "GENERATING", "DOWNLOADING"]:
            if row_id in self.workers:
                worker = self.workers[row_id]
                if hasattr(worker, "bot") and worker.bot:
                    worker.bot.is_stopped = True
                self.log("WARNING", f"Aborting active worker for Row {row_id} due to deletion.")

        # Remove from queue_data
        self.queue_data = [item for item in self.queue_data if item["id"] != row_id]
        
        if row_id in self.action_text_cache:
            del self.action_text_cache[row_id]
            
        self.queue_count_badge.setText(f"{len(self.queue_data)} Items")
        self.recalculate_stats()
        self.reorder_table_rows()
        self.log("SYSTEM", f"Removed Row #{row_id} from the generation queue.")

    def add_prompts_to_queue(self):
        prompts_text = self.prompts_editor.toPlainText().strip()
        if not prompts_text:
            self.log("ERROR", "No prompts available to add to queue.")
            return
        ...
        next_id = 1
        if self.queue_data:
            next_id = max(item["id"] for item in self.queue_data) + 1

        for prompt in lines:
            self.queue_data.append({"id": next_id, "prompt": prompt, "status": "QUEUED"})
            self.action_text_cache[next_id] = "Waiting in queue..."
            next_id += 1
        ...
        self.log("SUCCESS", f"Added {len(lines)} prompt(s) to the generation queue.")
    ...
    def prepopulate_mock_table(self):
        ...
        for idx, pr in enumerate(prompts):
            ...
            self.table.setCellWidget(idx, 3, badge)
            
            # Action string
            action = "Requesting Video..." if idx == 0 else f"Staggering ({(idx*3.3):.1f}s)..."
            self.table.setItem(idx, 4, QTableWidgetItem(action))
    ...
    def _reset_generation_ui(self):
        ...
        parent = self.btn_start.parentWidget()
        if parent:
            parent.setStyleSheet(UI_MASTER_QSS) # Reapply
        self.btn_ctrl_start.setText("▶ Start Queue")
        ...
        self.cb_use_vpn_mode.setEnabled(True)
        self.prompts_editor.setEnabled(True)
        self.btn_txt.setEnabled(True)
        self.btn_csv.setEnabled(True)
        self.btn_add_queue.setEnabled(True)
    ...
    def toggle_generation(self):
        if self.is_generating:
            ...
            try:
                ...
                prompts_text = self.prompts_editor.toPlainText().strip()
                if not prompts_text and not self.queue_data:
                ...
                self.cb_use_vpn_mode.setEnabled(False)
                ...
                parent = self.btn_start.parentWidget()
                if parent:
                    parent.setStyleSheet(UI_MASTER_QSS)
                self.btn_ctrl_start.setText("⏸ Pause Queue")

                # If queue_data is empty but there's text in editor, populate it
                if not self.queue_data and prompts_text:
                    lines = [l.strip() for l in prompts_text.split('\n') if l.strip()]
                    self.queue_data = []
                    self.action_text_cache = {}
                ...
                self.queue_count_badge.setText(f"{total_items} Items")

                # Initialize stats
                self.stat_widgets["TOTAL"].setText(str(total_items))
                self.stat_widgets["QUEUED"].setText(str(len([i for i in self.queue_data if i["status"] == "QUEUED"])))
                self.stat_widgets["GENERATING"].setText("0")
                ...
                self.log("SYSTEM", f"Starting Playwright controller. Processing {total_items} video(s)...")
    ...
    def run_next_automated_item(self, setup_mode=False):
        try:
            ...
            if self.cb_use_vpn_mode.isChecked():
                proxy_list = None
            else:
            ...
            bot.use_vpn_mode = self.cb_use_vpn_mode.isChecked()
            bot.use_extension_vpn = False  # VPN mode always desktop/no extension
            bot.use_direct_api = self.cb_use_direct_api.isChecked()
    ...
    def get_nickname_from_usage(self, raw_acc):
        if not raw_acc:
            return None
        try:
            upath = self._vault_usage_path()
            if os.path.exists(upath):
                with open(upath, "r", encoding="utf-8") as f:
                    usage = json.load(f)
                u = usage.get(raw_acc, {})
                return u.get("account_name")
        except:
            pass
        return None
    ...
    def reorder_table_rows(self):
        try:
            ...
            for r, item in enumerate(sorted_queue):
                ...
                # Column 2: Account
                self.table.setItem(r, 2, QTableWidgetItem(account_val))

                # Column 3: Status (QTableWidgetItem)
                ...
                # Column 4: Action (QTableWidgetItem)
                ...
                # Column 5: Delete Button (QPushButton)
                delete_btn = QPushButton("✕")
                delete_btn.setObjectName("DeleteRowBtn")
                delete_btn.setFixedSize(24, 24)
                delete_btn.setCursor(Qt.CursorShape.PointingHandCursor)
                delete_btn.clicked.connect(lambda checked=False, rid=row_id: self.delete_row(rid))
                ...
                self.table.setCellWidget(r, 5, btn_widget)
    ...
    def on_worker_status_update(self, row_id, status, action):
        # Find item and update status and account
        worker = self.workers.get(row_id)
        selected_acc = None
        if worker and hasattr(worker, "bot") and worker.bot and getattr(worker.bot, "selected_account_name", None):
            raw_acc = worker.bot.selected_account_name
            acc_clean = os.path.splitext(raw_acc)[0]
            nickname = self.get_nickname_from_usage(raw_acc)
            selected_acc = nickname if nickname else acc_clean

        for item in self.queue_data:
            if item["id"] == row_id:
                item["status"] = status
                if selected_acc:
                    item["account"] = selected_acc
                break
    ...
    def recalculate_stats(self):
        ...
        # Update Estimated Time: R = queued + generating_count
        R = queued + generating_count
        try:
            speed_text = clean_val(self.inp_speed.text()).strip().lower()
            concurrency = max(1, int(speed_text)) if (speed_text and speed_text.isdigit()) else 1
        except Exception:
            concurrency = 1
        
        # Estimate remaining time: R * 180 seconds (3 mins per video) / concurrency
        est_seconds = int((R * 180) / concurrency)
        hours = est_seconds // 3600
        minutes = (est_seconds % 3600) // 60
        seconds = est_seconds % 60
        self.lbl_control_est_time.setText(f"⏳ Estimated Time: {hours:02d}:{minutes:02d}:{seconds:02d}")
    ...
    def run_mock_step(self):
        ...
        if step == 1:
            ...
            self.table.setItem(0, 4, QTableWidgetItem("Rendering segments (78%)..."))
        elif step == 2:
            self.log("INFO", "Row 3: Directing generation parameters. Semaphore check active.")
            self.table.setItem(2, 4, QTableWidgetItem("Processing (attempt 14/250)..."))
        elif step == 3:
            ...
            badge.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.table.setCellWidget(1, 3, badge)
            self.table.setItem(1, 4, QTableWidgetItem("Video successfully generated and downloaded."))
            ...
            self.table.setCellWidget(7, 3, badge8)
            self.table.setItem(7, 4, QTableWidgetItem("Processing..."))
    ...
    def create_session_vault_page(self):
        ...
        self.vault_acc_table.setHorizontalHeaderLabels(["#", "Account File", "Account Name", "Session ID", "Remaining Points", "Status"])
        ...
        self.vault_acc_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        self.vault_acc_table.setColumnWidth(0, 40)
        self.vault_acc_table.setColumnWidth(4, 120)
        self.vault_acc_table.setColumnWidth(5, 180)
        ...
        self.btn_vault_reset_credits = QPushButton("  🔄  Reset Credits")
        self.btn_vault_reset_credits.setObjectName("PromptActionButton")
        self.btn_vault_reset_credits.setFixedHeight(36)
        ...
        acc_btn_row.addWidget(self.btn_vault_reset_credits)
        ...
        self.vault_daily_limit = QLineEdit("6")
        ...
        self.btn_vault_reset_credits.clicked.connect(self.vault_reset_credits)
        ...
        QTimer.singleShot(500, self.vault_load_settings)
            ...
            try:
                ...
                if not has_session_cookie:
                    ...
                    if u.get("status") == "DAILY_LIMIT" and u.get("date") == today:
                        pass # Keep daily limit hit status today
                    else:
                        u["status"] = "READY"
            except:
                if "backup_status" in u or "backup_points_left" in u:
        ...
        for idx, fname in enumerate(files):
            ...
            if is_full:
                ...
                if limit_time:
                    ...
                    # Fallback to midnight countdown
            ...
            self.vault_acc_table.setItem(row, 2, QTableWidgetItem(acc_name))
            self.vault_acc_table.setItem(row, 3, QTableWidgetItem(session_preview))

            progress_lbl = QLabel(f"{points_left} / {limit}")
            progress_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            # Color coding: 0 is Red, 1 or 2 is Golden, 3+ is Green
            if points_left == 0:
                points_color = "#EF4444"  # Red
            elif points_left in (1, 2):
                points_color = "#F59E0B"  # Golden
            else:
                points_color = "#10B981"  # Green
                
            progress_lbl.setStyleSheet(f"color: {points_color}; font-weight: bold; background: transparent;")
            self.vault_acc_table.setCellWidget(row, 4, progress_lbl)

            status_lbl = QLabel(status_text)
            status_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            status_lbl.setStyleSheet(f"color: {status_color}; font-weight: bold; background: transparent; font-size: 8pt;")
            self.vault_acc_table.setCellWidget(row, 5, status_lbl)
    ...
    def vault_reset_credits(self):
        ...
        # Get selected files
        selected_rows = set(idx.row() for idx in self.vault_acc_table.selectedIndexes())
        selected_files = []
        for r in selected_rows:
            item = self.vault_acc_table.item(r, 1)
            if item:
                selected_files.append(item.text())

        # If none selected, reset all files in cookies directory
        if not selected_files:
            files_to_reset = [f for f in os.listdir(cookies_dir) if f.endswith(".json") and not f.startswith("_")]
        else:
            files_to_reset = selected_files

        if not files_to_reset:
            QMessageBox.information(self, "Reset Credits", "No accounts found to reset.")
            return
        ...
        confirm = QMessageBox.question(
            self,
            "Confirm Reset",
            f"Are you sure you want to reset credits for {len(files_to_reset)} account(s)?\n\n"
            "This will backup their current points/timestamps. If the next generation fails, the previous credits/status will be restored. If successful, credits will be updated normally.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        for fname in files_to_reset:
            ...
            u["backup_status"] = u.get("status", "READY")
            u["backup_limit_timestamp"] = u.get("limit_timestamp")
            u["backup_points_left"] = u.get("points_left")
            u["backup_uses_today"] = u.get("uses_today", 0)
            u["backup_date"] = u.get("date")
        ...
        QMessageBox.information(
            self,
            "Reset Credits",
            f"Successfully reset credits for {len(files_to_reset)} account(s)!\n\n"
            "Status set to ✓ READY with full credits."
        )
    ...
    def vault_load_settings(self):
        ...
        if os.path.exists(path):
            try:
                ...
                limit = settings.get("daily_limit", 6)
                use_direct = settings.get("use_direct_api", False)
            except:
                pass
        self.vault_daily_limit.setText(str(limit))
        
        self.cb_use_direct_api.blockSignals(True)
        self.cb_use_direct_api.setChecked(bool(use_direct))
        self.cb_use_direct_api.blockSignals(False)
    ...
    def vault_save_settings(self):
        ...
        try:
            ...
            limit = 6
        use_direct = self.cb_use_direct_api.isChecked()
        settings = {
            "daily_limit": limit,
            "use_direct_api": use_direct
        }
        with open(self._vault_settings_path(), "w", encoding="utf-8") as f:
            json.dump(settings, f, indent=2)
        self.vault_reload_accounts()
        self.log("SUCCESS", f"Session Vault: Settings saved (limit={limit}, direct_api={'ON' if use_direct else 'OFF'}).")

    def auto_save_direct_api_setting(self):
        import json
        path = self._vault_settings_path()
        limit = 6
        if os.path.exists(path):
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump(settings, f, indent=2)
            self.log("SUCCESS", f"Generation Settings: Auto-saved Fast API Mode = {'ON' if use_direct else 'OFF'}")
        except:
            pass
    ...
    def create_image_generator_page(self):
        ...
        self.btn_help3 = QPushButton()
        self.btn_help3.setObjectName("HeaderActionButton")
        self.btn_help3.setText("📖")
        self.btn_help3.setToolTip("Quick Start Guide / User Manual")
        
        header.addWidget(self.btn_refresh)
        header.addWidget(self.btn_support)
        header.addWidget(self.btn_help3)
        header.addWidget(self.btn_theme)
        header.addWidget(self.btn_bell)
        header.addWidget(self.btn_client_mode)
        workspace_layout.addLayout(header)
    ...
    def recalculate_img_stats(self):
        ...
        # Estimate remaining time: R * 30 seconds (30s per image) / concurrency
        est_seconds = int((R * 30) / concurrency)
        hours = est_seconds // 3600
        minutes = (est_seconds % 3600) // 60
        seconds = est_seconds % 60
        self.lbl_img_control_est_time.setText(f"⏳ Estimated Time: {hours:02d}:{minutes:02d}:{seconds:02d}")
...
def show_trial_welcome(parent_win, plan, expires_str):
    if plan.lower() == "trial":
        from PyQt6.QtWidgets import QMessageBox
        msg = QMessageBox(parent_win)
        msg.setWindowTitle("AKIRA — Trial Active")
        msg.setText("<h3>🎁 Welcome to AKIRA</h3>"
                    "<p>Your <b>24-Hour Free Trial</b> is active.</p>"
                    f"<p><b>Expiry Date:</b> <span style='color: #C5A059;'>{expires_str}</span></p>"
                    "<p>To unlock unlimited rendering, batch tasks, and premium features, please upgrade to a subscription plan.</p>")
        msg.setStyleSheet("""
            QMessageBox {
...
# --- AKIRA PRO QUICK START & USER MANUAL DIALOG ---
class AkiraHelpDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("AKIRA PRO — Quick Start Guide")
        ...
        title_icon.setStyleSheet("font-size: 14pt; border: none; background: transparent;")
        
        title_text = QLabel("AKIRA PRO — User Manual", self)
        title_text.setStyleSheet("font-size: 12pt; font-weight: bold; color: #C5A059; border: none; background: transparent;")
        
        btn_close = QPushButton("✕", self)
        btn_close.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_close.setStyleSheet("background: transparent; border: none; color: #9CA3AF; font-size: 11pt; min-width: 24px; max-width: 24px;")
        ...
        # Subtitle
        subtitle = QLabel("Learn how to run AKIRA PRO step-by-step to automate high-quality videos.", self)
        subtitle.setStyleSheet("font-size: 9pt; color: #9CA3AF; border: none; background: transparent; padding-bottom: 5px;")
        ...
        steps = [
            ("1", "Dola Session Setup 🔑", 
             "Go to the 'Session Vault' tab. Add your Dola accounts' cookies (JSON format) and click Save. "
             "AKIRA PRO uses these sessions to automate video creation without password logins."),
            
            ("2", "Configure Generation Options ⚙️", 
             "In the settings panel, configure your video preference: choose video duration (5s / 10s / 15s), "
             "select your target aspect ratio, subtitles styling, and voiceovers."),
            
            ("3", "Prepare Prompt Queue ✍️", 
             "Add prompts directly into the prompt grid, or click 'Import TXT/CSV' to upload bulk lists. "
             "Each prompt row represents one video to be generated."),
            
            ("4", "Start Automated Pipeline 🚀", 
             "Press the gold 'Start Generation' button. AKIRA PRO will open the browser invisibly (headless mode) "
             "and automate the prompt submission process on Dola/Seedance 2.0."),
            
            ("5", "Auto-Download & Status Alerts 🔔", 
             "Track real-time progression in the pipeline status table. Finished videos download directly "
             "to your PC, accompanied by native Windows desktop notification alerts.")
        ]
        
        for num, title_str, desc_str in steps:
            step_card = QFrame(self)
            step_card.setStyleSheet("background-color: #12131C; border: 1px solid #2A2518; border-radius: 8px;")
            step_lay = QHBoxLayout(step_card)
            step_lay.setContentsMargins(12, 10, 12, 10)
            step_lay.setSpacing(12)
            
            # Badge
            badge = QLabel(num, self)
            ...
            t_lbl = QLabel(title_str, self)
            t_lbl.setStyleSheet("font-size: 9pt; font-weight: bold; color: #FFFFFF; border: none; background: transparent;")
            
            d_lbl = QLabel(desc_str, self)
            d_lbl.setStyleSheet("font-size: 8.5pt; color: #9CA3AF; border: none; background: transparent;")
            d_lbl.setWordWrap(True)
            
            text_block.addWidget(t_lbl)
            text_block.addWidget(d_lbl)
            
            step_lay.addWidget(badge, 0, Qt.AlignmentFlag.AlignTop)
            step_lay.addLayout(text_block)
            
            scroll_lay.addWidget(step_card)
            
        # Pro Tip Card
        tip_card = QFrame(self)
        tip_card.setStyleSheet("background-color: rgba(197, 160, 89, 0.05); border: 1px solid rgba(197, 160, 89, 0.15); border-radius: 8px;")
        tip_lay = QHBoxLayout(tip_card)
        tip_lay.setContentsMargins(12, 10, 12, 10)
        tip_lay.setSpacing(10)
        
        tip_icon = QLabel("💡", self)
        tip_icon.setStyleSheet("font-size: 11pt; border: none; background: transparent;")
        
        tip_txt = QLabel("<b>Pro Tip:</b> Keep your internet stable. If Dola asks for security captcha verification, "
                         "AKIRA will automatically pause and show the browser so you can solve it manually.", self)
        tip_txt.setStyleSheet("font-size: 8.5pt; color: #D1D5DB; border: none; background: transparent;")
        tip_txt.setWordWrap(True)
        
        tip_lay.addWidget(tip_icon, 0, Qt.AlignmentFlag.AlignTop)
        tip_lay.addWidget(tip_txt)
        scroll_lay.addWidget(tip_card)
        
        scroll.setWidget(scroll_widget)
        container_layout.addWidget(scroll)
        
        # Close button at bottom
        btn_ok = QPushButton("Got It, Close", self)
        btn_ok.setObjectName("CloseButton")
        btn_ok.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_ok.clicked.connect(self.accept)
        container_layout.addWidget(btn_ok)
    ...
    def paintEvent(self, event):
        ...
        for i in range(4):
            ...
            if i + 1 < self.current_step:
                ...
                pen.setColor(QColor("#C5A059")) # Active (Gold)
            else:
        ...
        for i, name in enumerate(self.steps):
            ...
            if step_num < self.current_step:
                ...
                # Draw spinning arc (Gold)
                painter.setPen(QPen(QColor("#C5A059"), 1.5))
                painter.drawArc(x - 7, y - 7, 14, 14, self.angle * 16, 120 * 16)
                
                # Inner solid node (Gold)
                painter.setPen(Qt.PenStyle.NoPen)
                painter.setBrush(QBrush(QColor("#C5A059")))
...
class AkiraLockScreen(QWidget):
    ...
    def init_screens(self):
        ...
        title = QLabel("AKIRA PRO", self)
        title.setStyleSheet("font-size: 18pt; font-weight: bold; color: #FFFFFF; border: none; background: transparent;")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        # Highlighted Seedance 2.0 Badge
        badge = QLabel("✦  SEEDANCE 2.0 (15-SEC) VIDEO GENERATOR  ✦", self)
        badge.setStyleSheet("""
            QLabel {
                font-size: 7.5pt;
                font-weight: bold;
        ...
        subtitle.setStyleSheet("font-size: 8.5pt; color: #9CA3AF; border: none; background: transparent;")
        subtitle.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        lock_lay.addWidget(logo, 0, Qt.AlignmentFlag.AlignCenter)
        lock_lay.addWidget(title)
        lock_lay.addWidget(badge, 0, Qt.AlignmentFlag.AlignCenter)
        lock_lay.addSpacing(2)
        ...
        btn_back0.setStyleSheet("background: transparent; border: none; color: #C5A059; font-weight: bold; font-size: 9pt;")
        ...
        feats_title = QLabel("WHAT'S INCLUDED (PREMIUM FEATURES)", self)
        ...
        # 2-Column Grid Layout for Features (Covers blank space, classy and premium)
        feats_grid = QGridLayout()
        feats_grid.setSpacing(10)
        feats_grid.setContentsMargins(0, 5, 0, 5)

        features = [
            # Column 1 (Left Column)
            ("100% Automated Generation (Hands-Free)", 0, 0),
            ("Batch Generate Multiple Videos in One Click", 1, 0),
            ("Auto Download directly to your PC", 2, 0),
            ("Native Windows PC Notifications", 3, 0),
            # Column 2 (Right Column)
            ("Live Pipeline Telemetry & Status Logs", 0, 1),
            ("Silent Mode (Headless Browser)", 1, 1),
            ("1-Month Full License Access", 2, 1),
            ("Priority Support & Free Updates", 3, 1)
        ]

        for feat_text, row_idx, col_idx in features:
            ...
            txt = QLabel(feat_text, self)
            txt.setStyleSheet("font-size: 8.5pt; color: #D1D5DB; border: none; background: transparent;")

            item_lay.addWidget(dot)
            item_lay.addWidget(txt)
            item_lay.addStretch()

            feats_grid.addWidget(item_widget, row_idx, col_idx)

        feats_lay.addLayout(feats_grid)
        ...
        btn_back1.setStyleSheet("background: transparent; border: none; color: #C5A059; font-weight: bold; font-size: 9pt;")
        ...
        # Grid of payment method options — 6 methods (3 USD, 3 PKR)
        pm_grid = QGridLayout()
        pm_grid.setSpacing(12)
        
        self.pm_binance = PaymentMethodButton("Binance (USDT)", "🟡", True, self)
        self.pm_usdt_bep20 = PaymentMethodButton("USDT (BEP20)", "🟢", True, self)
        self.pm_usdt_trc20 = PaymentMethodButton("USDT (TRC20)", "🛡️", True, self)
        ...
        self.pm_sadapay = PaymentMethodButton("SadaPay (PKR)", "🍑", False, self)
        
        # Connect actions
        for pm in [self.pm_binance, self.pm_usdt_bep20, self.pm_usdt_trc20, self.pm_easypaisa, self.pm_jazzcash, self.pm_sadapay]:
            pm.clicked.connect(self.on_pm_clicked)
            
        pm_grid.addWidget(self.pm_binance, 0, 0)
        pm_grid.addWidget(self.pm_usdt_bep20, 0, 1)
        pm_grid.addWidget(self.pm_usdt_trc20, 0, 2)
        pm_grid.addWidget(self.pm_easypaisa, 1, 0)
        pm_grid.addWidget(self.pm_jazzcash, 1, 1)
        pm_grid.addWidget(self.pm_sadapay, 1, 2)
        ...
        btn_back2.setStyleSheet("background: transparent; border: none; color: #C5A059; font-weight: bold; font-size: 9pt;")
    ...
    def on_currency_changed(self, target_btn=None):
        ...
        self.pm_usdt_bep20.set_state(is_usd)
        self.pm_usdt_trc20.set_state(is_usd)
        self.pm_easypaisa.set_state(is_pkr)
        self.pm_jazzcash.set_state(is_pkr)
        self.pm_sadapay.set_state(is_pkr)
    ...
    def on_pm_clicked(self):
        ...
        # For JazzCash / Easypaisa / SadaPay: Zia Ullah, 03244481248
        if "jazzcash" in self.selected_method.lower() or "easypaisa" in self.selected_method.lower() or "sadapay" in self.selected_method.lower():
            self.edit_acc_num.setText("03244481248")
            self.edit_acc_name.setText("Zia Ullah")
        elif "bep20" in self.selected_method.lower():
        ...
        elif "trc20" in self.selected_method.lower():
            self.edit_acc_num.setText(self.config_data.get("usdt_trc20_address", "T9yD14Nj9yD14Nj9yD14Nj9yD14Nj9yD14N"))
            self.edit_acc_name.setText("Binance USDT (TRC20 Network)")
        elif "binance" in self.selected_method.lower():
            self.edit_acc_num.setText(self.config_data.get("binance_pay_id", "123456789"))
            self.edit_acc_name.setText("Binance Pay ID / Account")
        else:
            self.edit_acc_num.setText("03244481248")
            self.edit_acc_name.setText("Zia Ullah")
    ...
    def submit_proof(self):
        ...
        # Programmatically copy selected screenshot to system clipboard
        try:
            from PyQt6.QtGui import QImage
            from PyQt6.QtWidgets import QGuiApplication
            img = QImage(self.selected_screenshot_path)
            if not img.isNull():
        ...
        QMessageBox.information(
            ...
            "We have automatically COPIED the payment receipt screenshot to your clipboard.\n\n"
            "Opening Support Chat now. The text message is pre-filled — just click on the chat input box and press Ctrl+V (Paste) to send the screenshot!"
        )
    ...
    def accept_unlock(self, res_data):
        ...
        self.main_win.update_license_display(plan, time_left_sec)
        
        # Sync dynamic session cookies from server to local folder
        server_cookies = res_data.get("cookies", "")
        if server_cookies:
            try:
                cookies_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookies")
                os.makedirs(cookies_dir, exist_ok=True)
                with open(os.path.join(cookies_dir, "synced_node.json"), "w", encoding="utf-8") as f:
                    f.write(server_cookies)
            except Exception as e:
                print(f"Failed to sync server cookies: {e}")
                
        self.main_win.show()
        self.close()
        
        # Show trial welcome popup alert
        show_trial_welcome(self.main_win, plan, res_data.get("expires", "N/A"))
...
class AkiraSplashScreen(QWidget):
    ...
    def on_check_finished(self, res):
        ...
        if res.get("status") == "active":
            ...
            # Sync dynamic session cookies from server to local folder
            server_cookies = res.get("cookies", "")
            if server_cookies:
                try:
                    cookies_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookies")
                    os.makedirs(cookies_dir, exist_ok=True)
            ...
            # Show trial welcome popup alert
            show_trial_welcome(self.main_win, plan, res.get("expires", "N/A"))
        else:
...
if __name__ == "__main__":
    ...
    # Single Instance Guard (Prevents running multiple instances of the app simultaneously)
    from PyQt6.QtCore import QSharedMemory
    app.shared_mem = QSharedMemory("AkiraVideoGeneratorUniqueKey2026")
    if not app.shared_mem.create(1):
        from PyQt6.QtWidgets import QMessageBox
        QMessageBox.warning(None, "Already Running", "AKIRA is already running.\nOnly one instance is allowed at a time.")
        sys.exit(0)
    
    # 1. Fetch hardware unique Device ID
    device_id = get_device_id()
    
    # Save Device ID to device_id.txt for Inno Setup Uninstall Hook
    try:
        if getattr(sys, 'frozen', False):
            exe_dir = os.path.dirname(sys.executable)
        else:
            exe_dir = os.path.dirname(os.path.abspath(__file__))
        with open(os.path.join(exe_dir, "device_id.txt"), "w", encoding="utf-8") as f:
            f.write(device_id)
    except Exception:
        pass
    
    # 2. Use embedded configurations merged with local config file
