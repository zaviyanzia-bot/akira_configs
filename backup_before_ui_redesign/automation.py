...
class DolaAutomationBot:
    ...
    def run_generation(self, row_id, prompt, model, duration, ratio, setup_mode=False, is_image=False, proxy_list=None, sms_key=None, sms_country="0"):
        ...
        if match:
            ...
            prompt = f'"{visual_prompt}", {match.group(2).strip()}'
        else:
            quoted_parts = re.findall(r'"([^"]*)"', prompt)
            if quoted_parts:
                visual_prompt = quoted_parts[0].strip()
                visual_prompt = clean_prompt_duration(visual_prompt)
                prompt = prompt.replace(f'"{quoted_parts[0]}"', f'"{visual_prompt}"', 1)
            else:
                if ',' in prompt:
        ...
        for page_retry in range(max_page_retries):
            try:
                ...
                if not is_image and ratio:
                    ...
                    try:
                        ratio_selected = page.evaluate(f"""() => {{
                            // Find and click the Ratio button to open dropdown
                            const allEls = Array.from(document.querySelectorAll('button, div, span, p'));
                            ...
                            if (ratioBtn) {{
                                ratioBtn.click();
                                return 'clicked_ratio_btn';
                            }}
                            return 'ratio_btn_not_found';
                        }}""")
                        self.log("INFO", f"Row {row_id}: Ratio button click result: {ratio_selected}")
                        page.wait_for_timeout(1000)
                        
                        if ratio_selected == 'clicked_ratio_btn':
                            # Now click the specific ratio option (e.g. "9:16")
                            ratio_option_clicked = page.evaluate(f"""() => {{
                                const allEls = Array.from(document.querySelectorAll('button, div, span, li, p, a'));
                                const target = '{ratio}';
                                ...
                                for (const option of matches) {{
                                    ...
                                    if (rect.width > 0 && rect.height > 0) {{
                                        option.click();
                                        return 'selected_ratio_' + target;
                                    }}
                                }}
                                return 'ratio_option_not_found';
                            }}""")
                            self.log("INFO", f"Row {row_id}: Ratio option result: {ratio_option_clicked}")
                ...
                if not is_image and duration and not is_15s:
                    ...
                    try:
                        dur_btn_click = page.evaluate(f"""() => {{
                            const allEls = Array.from(document.querySelectorAll('button, div, span, p'));
                            // Find existing selected duration button (5s/10s/15s) to open dropdown
                            const durBtn = allEls.find(el => {{
                                const t = (el.innerText || '').trim();
                                return t === '5s' || t === '10s' || t === '15s';
                            }});
                            if (durBtn) {{
                                durBtn.click();
                                return 'clicked_dur_btn';
                            }}
                            return 'dur_btn_not_found';
                        }}""")
                        self.log("INFO", f"Row {row_id}: Duration btn click: {dur_btn_click}")
                        page.wait_for_timeout(1000)
                        
                        if dur_btn_click == 'clicked_dur_btn':
                            dur_option_clicked = page.evaluate(f"""() => {{
                                const allEls = Array.from(document.querySelectorAll('button, div, span, li, p, a'));
                                const target = '{dur_label}';
                                const matches = allEls.filter(el => {{
                                    const t = (el.innerText || '').trim();
                                    return t === target || t.includes(target);
                                }});
                                for (const option of matches) {{
                                    ...
                                    if (rect.width > 0 && rect.height > 0) {{
                                        option.click();
                                        return 'selected_dur_' + target;
                                    }}
                                }}
                                return 'dur_option_not_found';
                            }}""")
                            self.log("INFO", f"Row {row_id}: Duration option result: {dur_option_clicked}")
                            page.wait_for_timeout(800)
                ...
                self.update_status(row_id, "GENERATING", "[4/5] Rendering video (up to 10 mins)...")
                self.log("INFO", f"Row {row_id}: Monitoring for 'Your video is ready' + video element (up to 10 mins, checking every second)...")
                ...
                # ----------------------------------------------------------------
                # COMBINED PHASE: Every second check BOTH ready text AND video src
                # Trigger download immediately on first detection - no wasted waiting
                # ----------------------------------------------------------------
                for i in range(600):  # 600s = 10 min max
                    if getattr(self, "is_stopped", False):
                        raise Exception("USER_ABORTED")
                    ...
                    try:
                        ready_check = page.evaluate("""() => {
                            ...
                            return bodyText.includes('your video is ready') || bodyText.includes('video is ready');
                        }""")
                        if ready_check and not video_ready_detected:
                            video_ready_detected = True
                            self.log("SUCCESS", f"Row {row_id}: 'Your video is ready' detected at {i}s!")
                    except:
                        pass

                    # Check 2: Video element src (works even before ready text appears)
                    try:
                        found_src = page.evaluate("""() => {
                            const videos = Array.from(document.querySelectorAll('video'));
                            const lastVideo = videos[videos.length - 1];
                            if (lastVideo) {
                                return lastVideo.getAttribute('src') || lastVideo.currentSrc ||
                                       (lastVideo.querySelector('source') ? lastVideo.querySelector('source').getAttribute('src') : null);
                            }
                            return null;
                        }""")
                        if found_src:
                    ...
                    # If BOTH text detected AND video src available → break immediately
                    if video_ready_detected and video_src:
                        self.log("SUCCESS", f"Row {row_id}: Video ready + src found at {i}s. Proceeding to download immediately!")
                        break

                    # If only text detected but no src yet → wait max 30 more seconds
                    if video_ready_detected and not video_src:
                        if i > 0 and (i % 30) == 0:
                            self.log("WARNING", f"Row {row_id}: Ready text seen but no video src yet ({i}s). Still scanning...")

                    # Progress log every 30 seconds
                    if i > 0 and i % 30 == 0 and not video_ready_detected:
                        self.log("INFO", f"Row {row_id}: Still waiting for video generation... ({i}/600s)")

                    page.wait_for_timeout(1000)

                if not video_ready_detected:
                    self.log("WARNING", f"Row {row_id}: 'Your video is ready' not detected after 600s. Still attempting download...")
                ...
                def try_all_download_methods(src_url):
                    """Try all 5 download methods and return True on first success."""

                    # METHOD 1: Browser Fetch (inherits auth cookies, works for blob/CDN URLs)
                    try:
                        self.log("INFO", f"Row {row_id}: Method 1 - Browser fetch (current page)...")
                    ...
                    # METHOD 2: New Tab Direct Download / Same-Origin Fetch
                    if src_url.startswith("http"):
                        new_tab = None
                        try:
                            self.log("INFO", f"Row {row_id}: Method 2 - New tab direct download/fetch...")
                            new_tab = context.new_page()
                            try:
                                with new_tab.expect_download(timeout=10000) as dl_info:
                                    new_tab.goto(src_url, timeout=30000)
                                dl = dl_info.value
                                dl.save_as(target_path)
                                if is_valid_video(target_path):
                                    self.log("SUCCESS", f"Row {row_id}: Method 2 (direct download) succeeded. Size: {os.path.getsize(target_path)} bytes.")
                                    return True
                            except:
                                pass
                            # Same-origin fetch fallback in new tab
                            try:
                                b64 = new_tab.evaluate("""async () => {
                                ...
                                with open(target_path, "wb") as f:
                                    f.write(video_data)
                                if is_valid_video(target_path):
                                    self.log("SUCCESS", f"Row {row_id}: Method 2 (same-origin fetch) succeeded. Size: {len(video_data)} bytes.")
                                    return True
                            except:
                                pass
                        except Exception as e:
                    ...
                    # METHOD 3: Python requests HTTP download (plain CDN URL)
                    if src_url.startswith("http") and not src_url.startswith("blob:"):
                        try:
                            self.log("INFO", f"Row {row_id}: Method 3 - Python requests HTTP download...")
                            import requests as req_lib
                            headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://www.dola.com/"}
                            resp = req_lib.get(src_url, headers=headers, timeout=60, stream=True)
                            ...
                            with open(target_path, "wb") as f:
                                for chunk in resp.iter_content(chunk_size=65536):
                                    if chunk:
                                        f.write(chunk)
                            if is_valid_video(target_path):
                                self.log("SUCCESS", f"Row {row_id}: Method 3 (requests) succeeded. Size: {os.path.getsize(target_path)} bytes.")
                                return True
                        except Exception as e:
                            self.log("WARNING", f"Row {row_id}: Method 3 failed: {str(e)[:80]}")

                    # METHOD 4: DOM Scanner - find .mp4 URL inside innerHTML
                    try:
                        ...
                        if mp4_url and mp4_url != src_url:
                            import requests as req_lib
                            headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://www.dola.com/"}
                            resp = req_lib.get(mp4_url, headers=headers, timeout=60, stream=True)
                            resp.raise_for_status()
                            with open(target_path, "wb") as f:
                                for chunk in resp.iter_content(chunk_size=65536):
                                    if chunk:
                                        f.write(chunk)
                            if is_valid_video(target_path):
                                self.log("SUCCESS", f"Row {row_id}: Method 4 (DOM scanner) succeeded. URL: {mp4_url[:80]}")
                                return True
                    except Exception as e:
                        self.log("WARNING", f"Row {row_id}: Method 4 failed: {str(e)[:80]}")

                    # METHOD 5: Video element src deep scan (all video/source tags)
                    try:
                        ...
                        for alt_src in (all_srcs or []):
                            ...
                            try:
                                ...
                                with open(target_path, "wb") as f:
                                    f.write(video_data)
                                if is_valid_video(target_path):
                                    self.log("SUCCESS", f"Row {row_id}: Method 5 (deep src scan) succeeded. Size: {len(video_data)} bytes.")
                                    return True
                            except:
                                pass
                    except Exception as e:
                ...
                # Try all download methods if we have a URL
                if video_src:
                    self.log("INFO", f"Row {row_id}: Starting download with all methods for: {video_src[:80]}...")
                    download_success = try_all_download_methods(video_src)

                # If still no success, try DOM scanner + video src scan without a pre-found URL
                if not download_success:
                    self.log("WARNING", f"Row {row_id}: No video_src pre-found. Trying DOM/video element fallback methods...")
                    try:
                        # DOM scanner without initial src
                        mp4_url = page.evaluate(r"""() => {
                            const html = document.documentElement.innerHTML;
                            const match = html.match(/https?:\/\/[^"'\s<>]+\.mp4[^"'\s<>]*/);
                            return match ? match[0] : null;
                        ...
                        if mp4_url:
                            self.log("INFO", f"Row {row_id}: DOM scanner found URL: {mp4_url[:80]}")
